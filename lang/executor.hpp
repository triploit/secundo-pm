            ...
            for (int i = 0; i < Tokens.size(); i++)
            {
                ...
                if (value == "func" && (i+2) < Tokens.size())
                {
                    if (Tokens[i+1].getType() == TokenType.TYPE_OTHER &&
                        ...
                        RealTokens.push_back(Token(Tokens[i+1].getValue(), TokenType.TYPE_FUNCTION, Tokens[i+2].getValue()));
                    else
                    {
                        std::cout << "EXECUTOR: PUT_TOGETHER: FUNC " << Tokens[i+1].getValue() << ": ERROR: FATAL_ERROR: Syntax error!" << std::endl;
                        exit(1);
                    }
                    i+=2;
                }
